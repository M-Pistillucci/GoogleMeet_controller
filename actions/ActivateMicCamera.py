...
class ActivateMicCamera(ActionCore):
    def __init__(self, *args, **kwargs):
        ...
        self.add_event_assigner(EventAssigner(
            id="activate_mic_camera_pressed",
            ui_label="Activate MicCamera",
    ...
    def update_status(self):
        ...
        if not self.camera_enabled and not self.mic_enabled:
    ...
    def on_pressed(self, data) -> None:
        ...
        if self.backend:
            if not bool(self.backend.get_mic_enabled()):
                self.backend.toggle_microphone()
            if not bool(self.backend.get_camera_enabled()):
